...
module.exports = {
    ...
    getAll: function(req, res, next) {
        let moviesList = [];
        movieModel.find({ user: req.body.userId}, function(err, movies){
            if (err){
                next(err);
            } else{
    ...
    deleteByName:
        ...
        movieModel.findOneAndRemove({name: req.params.movieName}, function(err, movieInfo){
            if(err)
                next(err);
            else {
    ...
    create: function(req, res, next) {
        movieModel.create({ name: req.body.name, released_on: req.body.released_on, user: req.body.userId }, function (err, result) {
            if (err)
                next(err);
            else
